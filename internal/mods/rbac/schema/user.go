...
type User struct {
	...
	Gender    int       `json:"gender"`                        // Gender of user (1-male, 2-female)
	Email     string    `json:"email" gorm:"size:128;"`        // Email of user
	Remark    string    `json:"remark" gorm:"size:1024;"`      // Remark of user
	Status    int       `json:"status"`                        // Status of user (1-activated, 0-freezed)
	CreatedAt time.Time `json:"created_at" gorm:"index;"`      // Create time
	UpdatedAt time.Time `json:"updated_at" gorm:"index;"`      // Update time
	Roles     Roles     `json:"roles" gorm:"-"`                // Roles of user
}
...
type UserQueryParam struct {
	...
	Gender       int    `form:"gender"`   // Gender of user (1-male, 2-female)
	Email        string `form:"email"`    // Email of user
	Phone        string `form:"phone"`    // Phone of user
	Status       int    `form:"status"`   // Status of user (1-activated, 2-freezed)
}
...
type UserForm struct {
	Username string `json:"username,omitempty" binding:"required,max=64"`        // Username for login
	Name     string `json:"name,omitempty" binding:"required,max=64"`            // Name of user
	Password string `json:"password,omitempty" binding:"max=64"`                 // Password for login (md5 hash)
	Phone    string `json:"phone,omitempty" binding:"max=32"`                    // Phone number of user
	Gender   int    `json:"gender,omitempty" binding:"required,oneof=1 2,max=2"` // Gender of user
	Email    string `json:"email,omitempty" binding:"max=128"`                   // Email of user
	Remark   string `json:"remark,omitempty" binding:"max=1024"`                 // Remark of user
	Status   int    `json:"status,omitempty" binding:"required,oneof=1 2,max=2"` // Status of user (1-activated, 0-freezed)
	Roles    Roles  `json:"roles,omitempty" binding:"required"`                  // Roles of user
}
